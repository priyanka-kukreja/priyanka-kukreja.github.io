...
population, still remains unanswered. Is there a way for us to leverage
...
Voila! I may not be able to provide the exact answer, but at least I can
...
of the population mean, we expect the population mean to follow the same distribution. Hence,
...
Hence, to get a 95% CI, we will exclude the tail on both sides of the curve each weighing about 2.5%. Hence, we are looking for the area corresponding to z<sub>p</sub> = 0.025.
...
derived for CI, this can be observed by increasing _n_, the size of
the sample.
